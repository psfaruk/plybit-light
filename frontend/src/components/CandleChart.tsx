...
export function CandleChart() {
  ...
  // Smooth interpolation to target candle values
  const animateToTarget = (target: { epoch: number; open: number; high: number; low: number; close: number }) => {
    if (!candleSeriesRef.current) return;

    if (animationRef.current?.rafId !== null && animationRef.current?.rafId !== undefined) {
      cancelAnimationFrame(animationRef.current.rafId);
    }

    const current = animationRef.current?.targetCandle ?? {
  ...
  useEffect(() => {
    ...
    const ema5 = chart.addLineSeries({
      color: "#00b4ff", lineWidth: 1, priceLineVisible: false, lastValueVisible: false,
    });
    const ema20 = chart.addLineSeries({
      color: "#b347ff", lineWidth: 1, priceLineVisible: false, lastValueVisible: false,
    });
  ...
  // Update candles — setData on dataset change, smooth animation on tick
  useEffect(() => {
    ...
    if (isNewDataset) {
      ...
      // Smooth interpolation to new tick values
      animateToTarget({
        epoch: last.epoch,
        open:  last.open,
        high:  last.high,
